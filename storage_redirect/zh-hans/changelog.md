...
- 重新设计及重构许多 UI 相关部分，降低理解难度，且更加流畅及稳定
- 增强模式 v22.0：增加禁止 `IVold#remountUid` 功能（在设置中称为“禁止重新挂载”）以避免隔离失效（在 Android 9 以上可用）
- 增强模式：加回“修复应用间交互”中的修改传递的文件路径功能，但暂时只处理 `ACTION_VIEW`，且不再经由存储重定向应用中转<sup>**〔1〕**</sup>
- 重新设计在线规则
  - 规则内的文字不再是只能使用应用内的预置文字
  - 添加更多可以展示提示文字的位置
  - 官方规则现在位于 `v2` 分支
  - 官方规则将从我们的服务器而非 GitHub 获取，主要目的是绕过 `raw.githubusercontent.com` 在中国大陆地区无法访问的问题

<br><sub>**〔1〕** 1. 触发反序列化 `extras` 很危险；2. 即使我们什么也不做，原始行为在 Android 10 上已经会造成崩溃，我们没有必要越庖代俎为劣质应用「修复问题」</sub>
