...
如果应用将用户文件保存至私有数据文件夹（如 `Android/data/com.example/files`），这意味着应用的开发者不希望用户直接使用这些文件或是它们做错了。你应该放弃你的需求或是要求应用的开发者做出更改。
