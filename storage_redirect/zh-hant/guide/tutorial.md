...
如果應用程式將使用者檔案儲存至私有資料資料夾（如 `Android/data/com.example/files`），這意味著應用程式的開發者不希望使用者直接使用這些檔案或是它們做錯了。你應該放棄你的需求或是要求應用程式的開發者做出更改。
